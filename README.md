...
#### Inputs typically are NESTED, `<input/>` inside a `<label> <input/> </label>` tag. Notifies to the browser the tags association, so when a label is clicked the browser focuses on associated input. <br/>
